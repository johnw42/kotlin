...
public class JvmCodegenUtil {
    ...
    public static boolean couldUseDirectAccessToProperty(
        ...
        if (!isDebuggerContext(context)) {
            // Unless we evaluating expression in debugger context, only properties of the same class can be directly accessed
            if (!isCallInsideSameClassAsDeclared(property, context)) return false;
        }
        else {
            // In debugger we want to access through accessors if they are present. If property override something accessors must be present.
