...
internal fun genFiles(file: File, fileContent: String, filesHolder: CodegenTestsOnAndroidGenerator.FilesWriter): FqName? {
    ...
    //val oldPackages = resultFiles.filter { !it.oldPackage.isRoot }.map { it.oldPackage.asString() }.toSet()
    if (resultFiles.size == 1) {
        val resultFile = resultFiles.first()
        filesHolder.addFile(file.name, patchClassForName(resultFile.newClassId, resultFile.oldPackage, resultFile.content).patchSelfImports(resultFile.newPackage))
        return resultFile.newClassId
    }
    else {
...
private fun createTestFiles(file: File, expectedText: String): List<CodegenTestCase.TestFile> {
    ...
    if (hasJava.get()) return emptyList()
    return files
}
